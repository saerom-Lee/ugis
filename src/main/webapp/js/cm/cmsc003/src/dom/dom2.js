...
CMSC003.DOM = (() => {
	return {
		...
		showSearchResult(json) {
			...
			const sarList = existing.sar;
			if (sarList) {
				for (let i = 0; i < sarList.length; i++) {
					const elem = sarList[i];
					console.log(elem);
					dataMap[`sar-${cas.length}`] = elem;
					sar.push(elem);
				}
			}
			...
			dd_satellite.append($('<label for="vido-id-satellite-all">').text(`위성영상(${landsat.length + kompsat.length + sentinel.length + cas.length + sar.length}건)`));
			...
			if (sar) {
				for (let i = 0; i < sar.length; i++) {
					...
					const dd = $('<dd class="ui-result-child-indent" title="' + elem.fullFileCoursNm + '">');
					const div = $('<div>');
					const check = $(`<input type="checkbox" class="js-search-result-sar" value="${i}" id="vido-id-sar-${i}">`);
					const label = $(`<label for="vido-id-sentinel-${i}">`).text(elem.vidoNm);
					const btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" value="${i}"><i class="fas fa-search"></i>미리보기</button>`);
					if (!elem.thumbnail) {
						$(btn).prop('disabled', true);
					}
					div.append(check).append(label).append(btn);
		...
		showSearchResultTree(json, panelElem, resetPanelElem) {
			...
			// 위성 - sar (satNm, map)
			const sarList = existing.satellite[4];
			if (sarList) {
				count = 0;
				const { satNm, map } = sarList;
				for (const e of map) {
					//					const {folder, fileList} = e;
					//					cas.map[folder] = [];
					//					for (const elem of fileList) {
					...
					for (const folder of folderList) {
						const { folderNm, fileList } = folder;
						sar.map[date][folderNm] = [];
						for (const elem of fileList) {
							//console.log('komp',elem);
							dataMap[`sar-${count}`] = elem;
							sar.map[date][folderNm].push({ index: count, elem: elem });
							count++;
						}
					}
				}
			...
			const satellite_span = $('<span>').text(`위성영상(${kompsat.length + sentinel.length + landsat.length + cas.length + sar.length}건)`);
			...
			if (landsat) {
				...
				//수정 - 2024.11.12 연도 트리 생성
				for (const date in landsat.map) {

					const year = date.match(/^\d{4}/)[0];
	
					//yearList값 확인 후 값 넣기
					 if (!yearList.includes(year)) { // 값이 존재하지 않으면
					       yearList.push(year); // 추가
					 }
				...
				for (var y=0; y<yearList.length; y++) {
					...
					const year_li = $(`<li class="js-tree-parent-3" value="${yearList[y]}" id="vido-id-satellite-title-landsat-${yearList[y]}">`);
					const year_span = $('<span>').text(yearList[y]);
					const year_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-landsat-image-layer" data-tree="vido-id-satellite-title-landsat-${yearList[y]}"><i class="fas fa-search"></i></button>`);
					const year_ul = $('<ul>');
					year_li.append(year_span);
					year_li.append(year_btn);
					year_li.append(year_ul);
					sat_ul.append(year_li);
					
					for (const date in landsat.map) {
						
						if (date.includes(yearList[y])) {
			...
			if (kompsat) {
				...
				for (var y=0; y<yearList.length; y++) {
					...
					const year_li = $(`<li class="js-tree-parent-3" value="${yearList[y]}" id="vido-id-satellite-title-kompsat-${yearList[y]}">`);
					const year_span = $('<span>').text(yearList[y]);
					const year_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-kompsat-image-layer" data-tree="vido-id-satellite-title-kompsat-${yearList[y]}"><i class="fas fa-search"></i></button>`);
					const year_ul = $('<ul>');
					year_li.append(year_span);
					year_li.append(year_btn);
					year_li.append(year_ul);
			...
			if (sentinel) {
				...
				for (var y=0; y<yearList.length; y++) {
					...
					const year_li = $(`<li class="js-tree-parent-3" value="${yearList[y]}" id="vido-id-satellite-title-sentinel-${yearList[y]}">`);
					const year_span = $('<span>').text(yearList[y]);
					const year_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sentinel-image-layer" data-tree="vido-id-satellite-title-sentinel-${yearList[y]}"><i class="fas fa-search"></i></button>`);
					const year_ul = $('<ul>');
					year_li.append(year_span);
					year_li.append(year_btn);
					year_li.append(year_ul);
					...
					for (const date in sentinel.map) {
						...
						if (date.includes(yearList[y])) {
								...
								for (const folder in sentinel.map[date]) {
									...
									////MSIL1C  MSIL2A 구조를 만들기 위한 상태값
									/*var mktr = true
									
									////MSIL1C  MSIL2A 트리 생성

									const MSIL1C_li = $(`<li class="js-tree-parent-4" value="${folder}-MSIL1C" id="vido-id-satellite-title-sentinel-${date}-${folder}-MSIL1C">`);
									const MSIL1C_span = $('<span>').text("MSIL1C");
									const MSIL1C_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sentinel-image-layer" data-tree="vido-id-satellite-title-sentinel-${date}-${folder}-MSIL1C"><i class="fas fa-search"></i></button>`);
									const MSIL1C_ul = $('<ul>');
									

									const MSIL2A_li = $(`<li class="js-tree-parent-4" value="${folder}-MSIL2A" id="vido-id-satellite-title-sentinel-${date}-${folder}-MSIL2A">`);
									const MSIL2A_span = $('<span>').text("MSIL2A");
									const MSIL2A_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sentinel-image-layer" data-tree="vido-id-satellite-title-sentinel-${date}-${folder}-MSIL2A"><i class="fas fa-search"></i></button>`);
									const MSIL2A_ul = $('<ul>');
									
									
									if(mktr){
										MSIL1C_li.append(MSIL1C_span);
										MSIL1C_li.append(MSIL1C_btn);
										MSIL1C_li.append(MSIL1C_ul);
										folder_ul.append(MSIL1C_li);
										
										MSIL2A_li.append(MSIL2A_span);
										MSIL2A_li.append(MSIL2A_btn);
										MSIL2A_li.append(MSIL2A_ul);
										folder_ul.append(MSIL2A_li);
									...
									for (const elem of fileList) {
										...
										//MSIL1C  MSIL2A
										var fmType = elem.elem.fullFileCoursNm
										const MSIL1C = "MSIL1C"
										const MSIL2A = "MSIL2A"
										...
										if(fmType.includes(MSIL1C)){
											...
											MSIL1C_ul.append(li.append(span).append(btn_sentinel));
											
											
										}else if(fmType.includes(MSIL2A)){
											const li = $(`<li class="js-search-result-sentinel js-tree-child-3" value="${elem.index}" id="vido-id-sentinel-${date}-${folder}-${elem.index}"  title="${elem.elem.fullFileCoursNm}">`);
											const span = $('<span>').text(elem.elem.vidoNm);
											const btn_sentinel = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sentinel-image-layer" value="${elem.elem.vidoNm}" data-tree="vido-id-sentinel-${date}-${folder}-${elem.index}"><i class="fas fa-search"></i></button>`);
											if (!elem.elem.thumbnail) {
											...
											MSIL2A_ul.append(li.append(span).append(btn_sentinel));
										}
												*/
									...
									for (const elem of fileList) {
										...
										if(fmType.includes(MSIL1C)){
											 span = $('<span>').text( "MSIL1C_"+ elem.elem.vidoNm);
										}else if(fmType.includes(MSIL2A)){
											 span = $('<span>').text( "MSIL2A_"+ elem.elem.vidoNm);
										}
			...
			if (cas) {
				...
				for (var y=0; y<yearList.length; y++) {
					...
					const year_li = $(`<li class="js-tree-parent-3" value="${yearList[y]}" id="vido-id-satellite-title-cas-${yearList[y]}">`);
					const year_span = $('<span>').text(yearList[y]);
					const year_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-cas-image-layer" data-tree="vido-id-satellite-title-cas-${yearList[y]}"><i class="fas fa-search"></i></button>`);
					const year_ul = $('<ul>');
					year_li.append(year_span);
					year_li.append(year_btn);
					year_li.append(year_ul);
			...
			//기존 데이터 - 위성영상(sar) 노드 추가
			if (sar) {
				const sat_li = $(`<li class="js-tree-parent-2" value="sar" id="vido-id-satellite-title-sar">`);
				const sat_span = $('<span>').text("Sar");
				const sat_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-satellite-title-sar"><i class="fas fa-search"></i></button>`);
				const sat_ul = $('<ul>');
				sat_li.append(sat_span);
				sat_li.append(sat_btn);
				sat_li.append(sat_ul);
				...
				for (var y=0; y<yearList.length; y++) {
					...
					const year_li = $(`<li class="js-tree-parent-3" value="${yearList[y]}" id="vido-id-satellite-title-sar-${yearList[y]}">`);
					const year_span = $('<span>').text(yearList[y]);
					const year_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-satellite-title-sar-${yearList[y]}"><i class="fas fa-search"></i></button>`);
					const year_ul = $('<ul>');
					year_li.append(year_span);
					year_li.append(year_btn);
					year_li.append(year_ul);
					...
					for (const date in sar.map) {
						...
						if (date.includes(yearList[y])) {
							const date_li = $(`<li class="js-tree-parent-3" value="${date}" id="vido-id-satellite-title-sar-${date}">`);
							const date_span = $('<span>').text(date);
							const date_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-satellite-title-sar-${date}"><i class="fas fa-search"></i></button>`);
							const date_ul = $('<ul>');
							date_li.append(date_span);
							date_li.append(date_btn);
							date_li.append(date_ul);
							...
							for (const folder in sar.map[date]) {
								const fileList = sar.map[date][folder];
								const folder_li = $(`<li class="js-tree-parent-4" value="${folder}" id="vido-id-satellite-title-sar-${date}-${folder}">`);
								const folder_span = $('<span>').text(folder);
								const folder_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-satellite-title-sar-${date}-${folder}"><i class="fas fa-search"></i></button>`);
								const folder_ul = $('<ul>');
								folder_li.append(folder_span);
								folder_li.append(folder_btn);
								folder_li.append(folder_ul);
								...
								for (const elem of fileList) {
									const li = $(`<li class="js-search-result-sar js-tree-child-3" value="${elem.index}" id="vido-id-sar-${date}-${folder}-${elem.index}"  title="${elem.elem.fullFileCoursNm}">`);
									const span = $('<span>').text(elem.elem.vidoNm);
									const btn_sar = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" value="${elem.elem.vidoNm}" data-tree="vido-id-sar-${date}-${folder}-${elem.index}"><i class="fas fa-search"></i></button>`);
									if (!elem.elem.thumbnail) {
										btn_sar.attr("disabled", true);
									}
									folder_ul.append(li.append(span).append(btn_sar));
								}
							}
						}
					}
		...
		showRequestResult(json) {
			...
			const satelliteTitle = $('<dt>').text(`위성영상(${landsat.length + kompsat.length + sentinel.length + cas.length + sar.length}건)`);
			dl.append(satelliteTitle);
			...
			if (sar) {
				for (let i = 0; i < sar.length; i++) {
					...
					//					const label = $(`<labelC for="vido-id-sentinel-${i}">`).text(elem.vidoNm);
					const span = $('<span>').text(elem.vidoNm);
					const btn = $(`<button id="vido-id-request-sar-${i}" type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" value="${i}"><i class="fas fa-search"></i>미리보기</button>`);
					if (!elem.thumbnail) {
						$(btn).prop('disabled', true);
					}
					//					dd.append(check).append(label).append(btn);
...
//			}
			...
			if (sar) {
				const sat_li = $(`<li class="js-tree-parent-2" value="sar" id="vido-id-request-satellite-title-sar">`);
				const sat_span = $('<span>').text("Sar");
				const sat_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-request-satellite-title-sar"><i class="fas fa-search"></i></button>`);
				const sat_ul = $('<ul>');
				sat_li.append(sat_span);
				sat_li.append(sat_btn);
				sat_li.append(sat_ul);
				...
				for (const date in sar.map) {
					const date_li = $(`<li class="js-tree-parent-3" value="${date}" id="vido-id-request-satellite-title-sar-${date}">`);
					const date_span = $('<span>').text(date);
					const date_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-request-satellite-title-sar-${date}"><i class="fas fa-search"></i></button>`);
					const date_ul = $('<ul>');
					date_li.append(date_span);
					date_li.append(date_btn);
					date_li.append(date_ul);
					...
					for (const folder in sar.map[date]) {
						const fileList = sar.map[date][folder];
						const folder_li = $(`<li class="js-tree-parent-4" value="${folder}" id="vido-id-request-satellite-title-sar-${date}-${folder}">`);
						const folder_span = $('<span>').text(folder);
						const folder_btn = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" data-tree="vido-id-request-satellite-title-sar-${date}-${folder}"><i class="fas fa-search"></i></button>`);
						const folder_ul = $('<ul>');
						folder_li.append(folder_span);
						folder_li.append(folder_btn);
						folder_li.append(folder_ul);
						...
						for (const elem of fileList) {
							const li = $(`<li class="js-search-result-sar js-tree-child-3" value="sar-${elem.index}" id="vido-id-request-sar-${date}-${folder}-${elem.index}"  title="${elem.elem.fullFileCoursNm}">`);
							const span = $('<span>').text(elem.elem.vidoNm);
							const btn_sar = $(`<button type="button" class="btn btn-xs f-right ui-btn-preview-sar-image-layer" value="${elem.elem.vidoNm}" data-tree="vido-id-request-sar-${date}-${folder}-${elem.index}"><i class="fas fa-search"></i></button>`);
							if (!elem.elem.thumbnail) {
								btn_sar.attr("disabled", true);
							}
							folder_ul.append(li.append(span).append(btn_sar));
